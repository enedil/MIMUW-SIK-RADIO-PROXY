...
enum ChunkType { ICY_AUDIO = STDOUT_FILENO, ICY_METADATA = STDERR_FILENO, INTERRUPT };

class RadioReader {
  public:
    RadioReader(std::string const &host, std::string const &port,
                sockaddr_in const &address, std::string &resource, unsigned timeout,
                bool metadata);
    bool init();
